...
export default {
  data: function () {
    return {
      set: {
        name: "",
        oldschooljs_monster_id: 777, //****HARDCODED - TESTING ****
        head: {},
        body: {},
        neck: {},
        cape: {},
  ...
  methods: {
    createSet: function () {
      this.set.items = [];
      for (const key in this.set.test_items) {
      ...
      axios.post("/equipment-sets", this.set).then((response) => {
    ...
    retrieveImage: function (item) {
      ...
      image_url.slice(image_url.indexOf("#"), image_url.length - 1);
